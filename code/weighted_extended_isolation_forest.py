...
class iForest(object):
    ...
    def compute_paths(self, X_in = None):
        ...
        for i in  range(len(X_in)):
            ...
            mean = np.mean(h_list)
            quantile = list(np.quantile(h_list,[0.25,0.5,0.75]))
            depths = []

            if quantile[1]>mean:
                rate = 1 + 2*abs((quantile[-1]-mean)/mean)
                threshold = quantile[0]
                depths = [x*rate if x<=threshold else x for x in h_list]
                
            elif quantile[1]<mean:
                rate = 1 - 2*abs((quantile[-1]-mean)/mean)
            ...
            Eh = sum(depths)/self.ntrees                                             # Average of weight path length travelled by the point in all trees.
            S[i] = 2.0**(-Eh/self.c)
